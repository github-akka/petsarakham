...
class HomeComponent extends Component
{
    public function render()
    {
        ...
        $scategories = ServiceCategory::select('name', 'slug', 'image')->inRandomOrder()->take(8)->get();
        $f_service = DB::table('services')->select('name', 'slug', 'tagline', 'thumbnail', 'price',)->where('featured', 1)->inRandomOrder()->take(8)->get();
        $f_categories = ServiceCategory::select('name', 'slug', 'image')->where('featured', 1)->take(8)->get();
